...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const v1Length = Math.sqrt(x1 ** 2 + y1 ** 2);
  const v2Length = Math.sqrt(x2 ** 2 + y2 ** 2);
...
function getParallelepipedDiagonal(a, b, c) {
  const sumSqueres = a ** 2 + b ** 2 + c ** 2;
  return Math.sqrt(sumSqueres);
}
...
function toNumber(value, def) {
  ...
  if (
    (typeof converted === 'number' || converted instanceof Number) &&
